...
| :-------------: | :----: | :----------------------------------------------------------: |
|    cards.py     | xxx  |                将cards.json文件数据导入card表                |
| line_station.py | xxx  | 将lines.json文件中line_name和station_name的相关数据导入line_stations表 |
|    lines.py     | xxx  |            将lines.json文件中剩余数据导入lines表             |
|   station.py    | xxx  |      将stations.json文件部分与station直接相关的数据导入      |
|  passenger.py   | xxx  |           将passenger.json文件数据导入passenger表            |
|   bus_info.py   | xxx | 将stations.json文件中关于busName,chukou等信息导入bus_info表  |
|   out_info.py   | xxx  | 将stations.json文件中每个station_name对应的outt,textt等出口信息导入out_info表 |
|     ride.py     | xxx |    将ride.json文件数据导入passenger_ride和card_ride两个表    |
